...
def player_plays(P_stats, player):
    ...
    decision = filter_hi(P_stats, res, player)
    ...
    # Joueurs pour le 6|1
        ...
        if P_stats.loc['cautious', player]:
            ...
            greed = filter_uncautious_greed(P_stats, choix, res, player)
            if greed == ['X']:
                pass
            else:
                for i in greed:
    ...
    if len(choix) == 6:
        ...
        decision = main_filter(P_stats, hidot_bet, choix, res, player)
        for d in decision:
            choix.append(d)
...
def player_attacks(P_stats, player, attack):
    victim = 'None'
    damages = 0
    pick = []
    pick_len = 0
    ...
    if P_stats.loc['attack', player] >= np.random.rand()*10:
        if victim_stats.loc['health'][victim_stats.loc['health'] < damages].count() > 1:
            victim = victim_stats.loc['health'][victim_stats.loc['health'] < damages].idxmax()
            return victim, damages
        else:
            victim = str(victim_stats.loc['health'].idxmin())
            return victim, damages
    else:
        victim = str(victim_stats.transpose().sample(1).index.to_list()[0])
        return victim, damages
